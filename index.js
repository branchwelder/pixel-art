...
function measure() {
  ...
  const availableY = bbox.height * devicePixelRatio;

  return Math.min(
    Math.floor(availableX / state.width),
    Math.floor(availableY / state.height)
  );
}

function measureOnResize(config) {
  let { width, height } = config;
  return {
    syncState(state) {
      if (width != state.width || height != state.height) {
        ({ width, height } = state);
        dispatch({ scale: measure() });
      }
    },
  };
}
...
window.onload = () => {
  ...
  if (isMobile()) {
    app = new App({
      ...
      dispatch,
      components: [
        layerPixel({ canvas: editorCanvas }),
        layerGrid({ canvas: gridCanvas }),
        layerOutline({ canvas: outlineCanvas }),
